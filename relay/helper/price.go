...
import (
	...
	"github.com/QuantumNous/new-api/pkg/promopricing"
...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (hosttypes.PriceData, error) {
	...
	promoDiscount, promoActive := promopricing.DiscountForModels(billingModelName, info.OriginModelName)

	// Check if this model uses tiered_expr billing
	if billing_setting.GetBillingMode(billingModelName) == billing_setting.BillingModeTieredExpr {
		return modelPriceHelperTiered(c, info, billingModelName, promptTokens, groupRatioInfo, promoDiscount, promoActive)
	}
	...
	if !usePrice {
		...
		preConsumeAmount := preConsumedTokens * ratio
		if promoActive {
			preConsumeAmount *= promoDiscount
		}
		quota, err := common.QuotaFromFloatStrict(preConsumeAmount)
		if err != nil {
			return hosttypes.PriceData{}, err
		}
	...
	if request, image := info.Request.(*dto.ImageRequest); image {
		...
		if !usePrice {
			imageAmount := priceData.ApplyOtherRatiosToFloat(info.ImageQuotaBeforeGroup * groupRatioInfo.GroupRatio)
			if promoActive {
				imageAmount *= promoDiscount
			}
			quota, err := common.QuotaFromFloatStrict(imageAmount)
			if err != nil {
				return hosttypes.PriceData{}, err
			}
	...
	if usePrice {
		...
		if promoActive {
			quotaToPreConsume *= promoDiscount
		}
	...
	if promoActive && promoDiscount == 0 {
...
func ModelPriceHelperPerCall(c *gin.Context, info *relaycommon.RelayInfo) (hosttypes.PriceData, error) {
	groupRatioInfo := HandleGroupRatio(c, info)
	promoDiscount, promoActive := promopricing.DiscountForModels(info.OriginModelName)
	...
	if usePrice {
		var err error
		amount := modelPrice * common.QuotaPerUnit * groupRatioInfo.GroupRatio
		if promoActive {
			amount *= promoDiscount
		}
		quota, err = common.QuotaFromFloatStrict(amount)
		if err != nil {
			return hosttypes.PriceData{}, err
		}
		if !operation_setting.GetQuotaSetting().EnableFreeModelPreConsume {
		...
		// 按量计费：以模型倍率的一半作为预扣额度
		var err error
		amount := modelRatio / 2 * common.QuotaPerUnit * groupRatioInfo.GroupRatio
		if promoActive {
			amount *= promoDiscount
		}
		quota, err = common.QuotaFromFloatStrict(amount)
...
func modelPriceHelperTiered(c *gin.Context, info *relaycommon.RelayInfo, billingModelName string, promptTokens int, groupRatioInfo hosttypes.GroupRatioInfo, promoDiscount float64, promoActive bool) (hosttypes.PriceData, error) {
	...
	preConsumedQuota, err := billingexpr.QuotaRoundStrict(quotaBeforeGroup * groupRatioInfo.GroupRatio * promoDiscount)
	if err != nil {
		return hosttypes.PriceData{}, err
	}
	...
	snapshot := &billingexpr.BillingSnapshot{
		...
		EstimatedQuotaBeforeGroup: func() float64 {
			if promoActive && promoDiscount > 0 {
