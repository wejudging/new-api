...
func UserCheckin(userId int) (*Checkin, int, error) {
	...
	tickets := operation_setting.GetCheckinDailyDraws()
	...
	if common.UsingMainDatabase(common.DatabaseTypeSQLite) {
		// SQLite 不支持嵌套事务，使用顺序操作 + 手动回滚
		return userCheckinWithoutTransaction(checkin, userId, tickets)
	}

	// MySQL 和 PostgreSQL 支持事务，使用事务保证原子性
	return userCheckinWithTransaction(checkin, userId, tickets)
}
...
func userCheckinWithoutTransaction(checkin *Checkin, userId int, tickets int) (*Checkin, int, error) {
	// 步骤1: 创建签到记录
	// 数据库有唯一约束 (user_id, checkin_date)，可以防止并发重复签到
